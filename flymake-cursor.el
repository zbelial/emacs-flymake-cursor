;;; flymake-cursor.el --- Show flymake messages in the minibuffer after delay   -*- lexical-binding: t; -*-
;;
...
(defcustom flymake-cursor-error-display-delay 1.5
...
the mode directly."
  ...
  (cond
   ...
   (t
    ...
    (remove-hook 'post-command-hook 'flymake-cursor-show-errors-at-point-pretty-soon)
    )))
...
(defun flymake-cursor-get-errors-at-point ()
  ...
  (let ((line-err-info-list (flymake-diagnostics (point))))
  ...
  (when flymake-cursor-mode
    (when flymake-cursor-errors-at-point
      (if (flymake-cursor-safe-to-display)
          (message "%s" (mapconcat 'flymake--diag-text flymake-cursor-errors-at-point "\n"))
        ))))
...
second, does the flymake error message (if any) get displayed."
  ...
  (when flymake-cursor-errors-at-point
    (setq flymake-cursor-error-display-timer
          (run-with-idle-timer flymake-cursor-error-display-delay nil 'flymake-cursor-show-stored-errors-now))))
